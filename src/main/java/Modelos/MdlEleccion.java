...
public class MdlEleccion {
    ...
    public ClsMensaje AgregarEleccion(ClsEleccion eleccion) {
        ...
        try {
            ...
            //Falta colocar el ganador también
    ...
    public ClsMensaje ActualizarEleccion(ClsEleccion eleccion) {
        ...
        try {
            ...
            sentencia.setString(5, String.valueOf(eleccion.getIdEleccion()));

            int resultado = sentencia.executeUpdate();

            if (resultado == 1) {
                mensaje.CambiarMensaje(mensaje.OK, "Has editado la eleccion con id: " + eleccion.getIdEleccion());
            } else {
                mensaje.CambiarMensaje(mensaje.ERROR, "Error no encontrado");
            }
    ...
    public int calcularPosicionGanadorEleccion(int[] votosCandidatos){
        int mayor = votosCandidatos[0];
    ...
    public void actualizarGanador(int idEleccion, String ganador){
        try {
            
            System.out.println("suuuu: " + idEleccion);
            System.out.println("suuuu2" + ganador);
            String sql = "UPDATE tbl_elecciones SET ganador = ? where id_eleccion = ?" ;            
            PreparedStatement sentencia = this.jdbc.conexion.prepareStatement(sql);
            sentencia.setString(1, ganador);
            sentencia.setInt(2, idEleccion);

            int resultado = sentencia.executeUpdate();
            System.out.println("xxxxxxxxxx resultado " + resultado);
            
            if (resultado == 1) {
                System.out.println("el resultado de la consulta por if fue: " + resultado);
            } else {
                System.out.println("el resultado de la consulta por else fue: " + resultado);
            }
